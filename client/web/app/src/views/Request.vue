<template>
  <v-container fluid class="h-100 overflow-y-auto">
    <v-row direction="column">
      <v-card tile right class="col-12 col-xs-10 col-sm-8 col-md-7 col-lg-5 col-xg-4 mx-auto">
        ...
        <v-form ref="form" @keypress.native.enter="submit">
          <v-container>
            ...
            <v-row v-if="addButton">
              <v-col cols="3">
                <v-btn @click="addProduct(request.products.length - 1)">Adicionar</v-btn>
              </v-col>
            </v-row>
            <v-row>
              <v-col cols="6">
            ...
            <v-row>
              <v-col>
                <v-textarea auto-grow filled label="Observações" solo @keypress.native.enter.stop></v-textarea>
              </v-col>
            </v-row>
          </v-container>
        </v-form>
        <v-card-actions>
          <v-btn color="primary" @click="submit">Solicitar</v-btn>
          <v-btn text color="primary" @click="clear">Cancelar</v-btn>
        </v-card-actions>
      </v-card>
    </v-row>
  </v-container>
...
export default {
  ...
  computed: {
    ...
    addButton() {
      return Boolean(
        this.request.products[this.request.products.length - 1].product || false
      );
    }
  },
  methods: {
    ...
    async loadProducts() {
      if (this.productsSugestions || this.products === []) {
        ...
        app.categories.loadList().then(list => {
    ...
    async removeVoidProduct(index: number) {
      if (
        !this.addButton &&
        index === this.request.products.length - 1 &&
        index > 0
      ) {
